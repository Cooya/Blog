...
const map = require('./extensions/map');
const tooltip = require('./extensions/tooltip');
...
async function convertFile(srcFolder, fileName) {
	...
	if (!fileContent) fileContent = (await readFile(post.markdown_file_path)).toString();
	...
	if (postDataAndContent.header) {
		...
		for (let attr in postDataAndContent.header) post[attr] = postDataAndContent.header[attr];
	}
...
function createConverter(settings) {
	...
	if (settings.extensions) {
		if (settings.extensions.includes('gallery')) {
			if (!settings.pictures_folder_url) throw Error('If gallery extension is enabled, the attribute "pictures_folder_url" must be specified.');
			if (!settings.picture_widths) throw Error('If gallery extension is enabled, the attribute "picture_widths" must be specified.');

			options.extensions.push(gallery(settings.pictures_folder_url, settings.picture_widths));
		}
		if (settings.extensions.includes('tooltip')) options.extensions.push(tooltip());
		if (settings.extensions.includes('map')) options.extensions.push(map());
	}
...
function determinePostId(relativeFolderPath, fileName) {
	if (fileName == 'index' || fileName == 'index.md') return relativeFolderPath || '/';
	if (relativeFolderPath) return relativeFolderPath + '/' + fileName.replace('.md', '');
